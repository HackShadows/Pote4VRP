...
def intra_relocate(trajet :Trajet) -> tuple[float, tuple[int, int]] :
	...
	for i in range(nb) :
		...
		for j in range(nb+1) :
			...
			if dist_tmp2 < mini and (not trajet.contraintes["temps"] or trajet.maj_horaires(
						modifie=False, 
		...
		for x, t2 in enumerate(trajets) :
			if x == i :
				...
				for j, c in enumerate(t1.clients) :
					...
					if t1.contraintes["temps"] and not t1.maj_horaires(indice=j, modifie=False): continue
					for y in range(t2.nb_clients+1) :
						if (not t2.contraintes["capacite"] or t2.marchandise + c.demande <= flotte.capacite) and (not t2.contraintes["temps"] or t2.maj_horaires(indice=y, client=c, modifie=False)) :
...
def intra_exchange(trajet :Trajet) -> tuple[float, tuple[int, int]] :
	...
	for i in range(nb-1) :
		...
		if tmp < mini and (not trajet.contraintes["temps"] or trajet.maj_horaires(
					modifie=False, 
			...
			if x == 0 :
				...
				for j, c1 in enumerate(t1.clients) :
					for y, c2 in enumerate(t2.clients) :
						if not t1.contraintes["capacite"] and not t2.contraintes["capacite"] or t1.marchandise - c1.demande + c2.demande <= flotte.capacite and t2.marchandise - c2.demande + c1.demande <= flotte.capacite :
							tmp = t1.dist_remplacer_client(j, c2) + t2.dist_remplacer_client(y, c1)
							if tmp < mini and (not t2.contraintes["temps"] or t2.maj_horaires(modifie=False, liste_clients=[c1]+t2.clients[y+1:], horaires=t2.horaires[:y])) \
										and (not t1.contraintes["temps"] or t1.maj_horaires(modifie=False, liste_clients=[c2]+t1.clients[j+1:], horaires=t1.horaires[:j])) :
								mini = tmp
								ind = ((i, j), (x+i, y))
...
def deux_opt(trajet :Trajet) -> tuple[float, tuple[int, int]] :
	...
	for i in range(nb-1) :
		...
		for j in range(i+2, nb+1) :
			...
			if dist < mini and (not trajet.contraintes["temps"] or trajet.maj_horaires(
						modifie=False, 
		...
		for j in range(1, nb1) :
			...
			for x, t2 in enumerate(trajets[i+1:]) :
				...
				for y in range(1, nb2) :
					...
					if not t1.contraintes["capacite"] and not t2.contraintes["capacite"] or t1.marchandise - marchandise1 + marchandise2 <= flotte.capacite and t2.marchandise - marchandise2 + marchandise1 <= flotte.capacite :
						tmp = dist_echanger_tab_clients((t1, j), (t2, y))
						if tmp < mini and (not t2.contraintes["temps"] or t2.maj_horaires(modifie=False, liste_clients=t1.clients[:j+1]+t2.clients[y+1:], horaires=[])) \
									and (not t1.contraintes["temps"] or t1.maj_horaires(modifie=False, liste_clients=t2.clients[:y+1]+t1.clients[j+1:], horaires=[])) :
							mini = tmp
							ind = ((i, j), (x+i+1, y))
